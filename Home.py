...
for index,row in dff.iterrows():
    with st.expander(row['Naziv ']):
        st.markdown('<p style="margin:5px;display:inline;float:left" class="blog-label">'+str(row['Usluga'])+'</p><p style="margin:5px;display:inline;float:left" class="blog-label">'+str(row['Korisnici'])+'<p style="margin:5px;display:inline;float:left" class="blog-label">'+str(row['Godine'])+'</p>', unsafe_allow_html=True)
        st.markdown('<p style="margin:0;display:inline;float:left" class="blog-label">'+str(row['Životna_dob'])+'</p>', unsafe_allow_html=True)
